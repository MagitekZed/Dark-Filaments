...
//
// Loaded as a plain <script> from file://. IIFE attaches to window.DF.sim.strategy.
// UMD shim at bottom for Node harness use.
//
...
(function (global) {
  ...
  function nextCost(upgrade, level) {
  ...
  function globalMult(stat, levels, upgrades) {
  ...
  function stackableVpc(name, state, params, upgrades, _classification) {
    ...
    function synMarginal(syn, providerLevel) {
      if (syn.kind === "additive") {
  ...
  function oneShotVpc(name, state, params, upgrades) {
    const u = find(upgrades, name);
    if (!u) return 0;
  ...
  function decideAction(state, params, upgrades) {
    ...
    const allCompletionistDone = isThresholdMode || cl.completionists.every(name => {
      const u = find(upgrades, name);
      return (state.levels[name] || 0) >= u.maxLevels;
    });
    ...
    function nextCompletionistCost() {
      let best = null;
      for (const name of cl.completionists) {
        const u = find(upgrades, name);
        const lvl = state.levels[name] || 0;
        if (lvl >= u.maxLevels) continue;
        const c = nextCost(u, lvl);
        if (c == null) continue;
    ...
    for (const name of cl.stackables) {
      ...
      if (postConsFocus && !find(upgrades, name).completionist) {
        if (!isLongSave) {
    ...
    if (bestAffName != null && maxAffVpc > 0) {
      return { action: "buy", upgrade: bestAffName, cost: costs[bestAffName] };
    }

    return { action: "none" };
  }

  global.DF.sim.strategy = {
    decideAction,
    classify,
    stackableVpc,
    oneShotVpc,
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = (typeof window !== 'undefined' ? window : globalThis).DF.sim.strategy;
}
