...
//
// Loaded as a plain <script> from file://. Pattern: IIFE attaches to window.DF.sim.data.
...
(function (global) {
  ...
  const DEFAULT_PARAMS = {
    ...
    cpmWindowMs: 30000,             // rolling window for live cpm stat
    autoCpmDefault: 100,            // dev-tool autoclicker default
    autoCpmMin: 1,                  // dev-tool autoclicker minimum
    autoCpmMax: 6000,               // dev-tool autoclicker maximum
    // Strategy / sim params.
  ...
  //   + baseMpc 1.0→0.00120. Mass values now in solar masses. v1 saves are
  //   numerically incoherent under v2 calibration (the stored mass would be
  //   applied at the wrong scale, putting any v1 player wildly past their
  //   intended tier).
  // v3 (2026-05-13 ladder renumber): T3 Dwarf Spheroidal insertion. Tier
  //   numbers above T2 shifted up by one (old T3 → new T4 Galactic Arm; old
  //   T4 → new T5 Galaxy; etc.). A v2 save's `currentTier: 3` would refer
  //   to old-T3 Galactic Arm but in the v3 engine T3 is Dwarf Spheroidal; the
  //   levels map keyed by upgrade name would survive but `currentTier`,
  //   `consolidationThreshold`, and the carry composition all become incoherent.
  //   save.js's deserializeState refuses v2 payloads with a clear error; the
  //   player starts a fresh universe. No automatic migration (the right
  //   re-mapping depends on whether the player had progressed PAST old-T2 at
  //   save time, and is ambiguous to invert in the boundary cases).
  // v4 (2026-05-13 consolidation rename): every engine identifier carrying
  //   the legacy `cohesion` name (state field, threshold/growth params,
  //   per-upgrade `cohesion` distribution field, the CONSOLIDATION_EPS guard,
  //   the log payload's `consolidation_after` shape, the per-tier
  //   `consolidationHitMs` snapshot, etc.) was renamed in-place to
  //   `consolidation`. The persisted payload shape is structurally identical
  //   to v3 except every `cohesion*` key is now `consolidation*`. A v3 save
  //   loaded under v4 would silently default every renamed field to 0, which
  //   silently breaks transition gates and click-bar fill (a v3 save's
  //   `cohesion: 2.4` would land as `consolidation: 0` in the v4 reader's
  //   path). REFUSED at load; the player starts a fresh universe. The rename
  //   has no numeric impact — engine math is unchanged.
  // save.js's deserializeState refuses v1, v2, and v3 payloads with a
  // clear error; the player starts a fresh universe.
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = (typeof window !== 'undefined' ? window : globalThis).DF.sim.data;
}
