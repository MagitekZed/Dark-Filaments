...
//      for each.
//
// Loaded as a plain <script> from file://. IIFE attaches to window.DF.sim.offline.
// UMD shim at bottom for Node test harness use (validate_offline.js).
//
// API contract:
...
(function (global) {
  ...
  const MAX_OFFLINE_TICKS = 30 * 24 * 3600;

  function deps() {
    const ns = global.DF && global.DF.sim ? global.DF.sim : null;
    if (ns && ns.core && ns.strategy && ns.runner && ns.data) {
      return { core: ns.core, strategy: ns.strategy, runner: ns.runner, data: ns.data };
    }
    if (typeof require !== 'undefined') {
      ...
      return { core, strategy, runner, data };
    }
    throw new Error('DF.sim.offline deps not loaded (need core, strategy, runner, data).');
  }
  ...
  function recomputeCarryAtTier(workingState, currentTier, allUpgrades, core, runner) {
    if (currentTier <= 1) {
    ...
    const cp = runner.composeCarryChain(
      ...
      currentTier, allUpgrades, { baseMpc: 1.0 }, core,
    );
    workingState.carry = {
  ...
  function consolidationThresholdForTier(tier, data) {
    const base = data.DEFAULT_PARAMS.consolidationThreshold;
    const growth = data.DEFAULT_PARAMS.consolidationGrowth;
  ...
  function reconstructFromOfflineWindow(savedState, elapsedSeconds, profileParams) {
    const { core, strategy, runner, data } = deps();
    ...
    let simTick = 0;
    for (simTick = 1; simTick <= ticks; simTick++) {
      ...
      const rates = core.computeRates(
        ws, activeUpgrades, carryForCompute,
  ...
  global.DF.sim.offline = {
    reconstructFromOfflineWindow,
    MAX_OFFLINE_TICKS,
  };
})(typeof window !== 'undefined' ? window : globalThis);

// UMD shim — for Node test harness use (validate_offline.js).
if (typeof module !== 'undefined' && module.exports) {
  module.exports = (typeof window !== 'undefined' ? window : globalThis).DF.sim.offline;
}
