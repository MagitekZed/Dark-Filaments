// Dark Filaments — T4 curve-shape analyzer (Node-only)
// Run via: node Prototype/src/test/t4_curve_shape.js [--scenario comp|thr] [--compare]
//
// Drives runSimulation through T1+T2+T3+T4 (Completion handoff, T4 mode by flag)
// and emits a per-decile MPS sample plus action mix.
//
// Two questions:
...
//
// Used to quantify "T4 feels like a slog" vs. "T4 feels like a steady climb".
...
function analyzeCurve(label, t4Result) {
  const trace = t4Result.trace;
  ...
  console.log('');
  console.log('=== ' + label + ' ===');
  console.log('  T4 total ticks: ' + total + '  (' + fmtTime(total) + ')');
  console.log('');
  ...
  console.log('    Lower = steadier climb.');
...
function compareT3() {
  console.log('');
  console.log('### T3 reference (Completion-handoff Comp) ###');
  const params = { cpm: 100, engagement: 1.0, saveVpcThreshold: 1.5 };
  ...
  const t3 = runner.runSimulation(params, { tier: 3, mode: 'completion', carryFrom: t2.finalState });
  analyzeCurve('T3 Completion (reference)', t3);
}
...
function main() {
  ...
  if (scenario === 'both' || scenario === 'comp') {
    ...
    analyzeCurve('T4 Comp-handoff Comp', chain.t4);
  }
  if (scenario === 'both' || scenario === 'thr') {
    const chain = runChain('completion', 'threshold');
    analyzeCurve('T4 Comp-handoff Thr', chain.t4);
  }

  if (compare) compareT3();
}
