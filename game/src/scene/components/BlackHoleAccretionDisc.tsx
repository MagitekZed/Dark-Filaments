...
export function BlackHoleAccretionDisc({ radius = 0.22 }: DiscProps) {
  ...
  const { state, positions, colors } = useMemo(() => {
    ...
    // Verbatim spike dep array — innerR/outerR already capture radius; the
    // extra `radius` dep is harmless (the memo rebuilds together regardless).
  ...
  return (
    ...
    <group rotation={BLACK_HOLE_AXIS_TILT}>
      <points>
