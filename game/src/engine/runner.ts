...
// literal + computation preserved byte-identical. computeCarryover /
// composeCarryChain keep their `core` parameter (offline.ts calls them with the
// real core module) — body uses the passed-in core exactly as the prototype did.
...
// Loose engine-internal types. The runner builds its own working state object
// (richer than EngineState — adds tick/time_s/thresholdHitTick) and merges
// arbitrary param overrides, so these stay deliberately wide to preserve the
// prototype's dynamic shapes byte-for-byte.
...
interface CarryPayload {
  ...
  carryMpc: number;
  carryAps: number;
  carry: { allMps: number; allMpc: number; allAps: number };
}
...
function buildTierConfig(tier: number, dataMod: typeof data): TierConfig {
  const base = (dataMod && dataMod.DEFAULT_PARAMS && dataMod.DEFAULT_PARAMS.consolidationThreshold != null)
    ? dataMod.DEFAULT_PARAMS.consolidationThreshold : 1.0;
  const growth = (dataMod && dataMod.DEFAULT_PARAMS && dataMod.DEFAULT_PARAMS.consolidationGrowth != null)
    ? dataMod.DEFAULT_PARAMS.consolidationGrowth : 2.5;
  return {
...
export const TIER_CONFIGS: Record<number, TierConfig> = (function () {
  const cfg: Record<number, TierConfig> = {};
  const TIERS_IMPLEMENTED = [1, 2, 3, 4, 5];
  for (const t of TIERS_IMPLEMENTED) cfg[t] = buildTierConfig(t, data);
  return cfg;
})();
...
export function upgradesForTier(allUpgrades: Upgrade[], tier: number): Upgrade[] {
...
export function computeCarryover(
  priorState: AnyState,
  priorUpgrades: Upgrade[],
  priorParams: AnyParams,
  coreMod: CoreModule,
  allUpgrades?: Upgrade[],
): CarryPayload {
  ...
  for (const u of priorUpgrades) {
    ...
    if (N === 0) continue;
    const syn = coreMod.synergyMult(u.name, priorState.levels, priorUpgrades, synProviders);
    baseMps += coreMod.selfContrib(N, u.baseMps, u.addMps, u.selfMps) * syn;
    baseMpc += coreMod.selfContrib(N, u.baseMpc, u.addMpc, u.selfMpc) * syn;
    baseAps += coreMod.selfContrib(N, u.baseAps, u.addAps, u.selfAps) * syn;
...
export function composeCarryChain(
  priorFinal: AnyState,
  currentTier: number,
  allUpgrades: Upgrade[],
  params: AnyParams,
  ...
  for (let priorT = 1; priorT < currentTier; priorT++) {
    ...
    for (const u of priorTUpgrades) {
      ...
      if (N === 0) continue;
      const syn = coreMod.synergyMult(u.name, priorFinal.levels, priorTUpgrades, synProviders);
      baseMps += coreMod.selfContrib(N, u.baseMps, u.addMps, u.selfMps) * syn;
      baseMpc += coreMod.selfContrib(N, u.baseMpc, u.addMpc, u.selfMpc) * syn;
      baseAps += coreMod.selfContrib(N, u.baseAps, u.addAps, u.selfAps) * syn;
      allMps *= Math.pow(u.allMps, N);
...
function initialState(_scenario: Scenario, upgrades: Upgrade[], tierCfg: TierConfig): AnyState {
...
function deepCopyLevels(levels: Record<string, number>): Record<string, number> {
...
//                    "completion".
export function runSimulation(params: AnyParams, scenario: Scenario): {
  headline: AnyState;
  trace: AnyState[];
  finalState: AnyState;
} {
  ...
  let carryPayload: CarryPayload | null = null;
  if (tier > 1) {
  ...
  const p: AnyParams = Object.assign({}, {
    cpm: 100,
    engagement: 1.0,
  ...
  for (let tick = 0; tick <= p.maxTicks; tick++) {
    ...
    const rates = core.computeRates(state as core.RateState, upgrades, carryForCompute, p, allUpgrades);
    ...
    let upgradeBought: string | null = null, costPaid: number | null = null, target: string | null = null;
    ...
    if (action.action === "transition") {
      ...
      if (u && state.mass >= (action.cost as number)) {
        state.mass -= (action.cost as number);
        state.levels[u.name] = (state.levels[u.name] || 0) + 1;
        state.consolidation += u.consolidation;
        upgradeBought = u.name;
        costPaid = action.cost as number;
      }
  ...
  // Build headline.
  const completionistDone = cl.completionists.every((name: string) => {
    const u = upgrades.find(x => x.name === name);
    return state.levels[name] >= (u as Upgrade).maxLevels;
  });
...
function traceRow(
  tick: number, state: AnyState, action: string, upgradeBought: string | null,
  costPaid: number | null, target: string | null,
  mpc: number, mps: number, aps: number, clickInc: number, passInc: number,
  autoInc: number, massIn: number, massOut: number,
): AnyState {
  const row: AnyState = {
    tick,
    time_s: tick,
