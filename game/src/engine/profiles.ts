...
//
// TS port (scaffold plan §5.1): IIFE wrapper + UMD shim stripped → ES exports.
// Pure data + helpers — no React/Three/DOM, so it satisfies engine purity and
// lives under engine/ alongside sweep.ts (its consumer). Not in the public
// barrel; the harnesses import it directly. Every value preserved verbatim.
...
export function timingHasDriftTarget(timingName: string | null): boolean {
...
export function activePhaseForDay(profile: TimingProfile | null, calendarDays: number): Phase | null {
...
export const REALISTIC_PAIRINGS: Pairing[] = [
...
export function lookupPairing(idOrName: string | null): Pairing | null {
...
export function primaryPairings(): Pairing[] {
...
export const ENGAGED_TARGETS: Record<string, Record<number, Target>> = {
  completion: {
...
export function totalTargetSecondsThroughTier(mode: string, tier: number): { low: number; high: number } | null {
...
export function describePairing(p: Pairing): {
  id: string;
  label: string;
  n: number;
  weight: string;
  dnfExpected: string | null;
