...
//
// TS port (scaffold plan §5.1): IIFE wrapper + UMD shim stripped; the JS bodies
// and every numeric literal/comment preserved. Types added over the shapes.
//
...
// Minimal state shape the rate functions read: just the levels map. The
// prototype passes bare { mass, consolidation, levels } objects (see
// validate_subhalo), so we type only what is read.
export interface RateState {
  levels: Record<string, number>;
}

// Carry payload shape consumed by computeRates. All fields optional; the
// prototype guards each with a null-coalescing default.
interface CarryLike {
  allMps?: number;
  allMpc?: number;
  allAps?: number;
...
export function selfContrib(N: number, B: number, A: number, S: number): number {
  if (N === 0) return 0;
...
export function computeRates(
  state: RateState,
  upgrades: Upgrade[],
  carry: CarryLike | null | undefined,
  params: BaseMpcParams | null | undefined,
  synergyProviders?: Upgrade[],
): Rates {
  const providers = synergyProviders || upgrades;
  const c = carry || {};
...
export function cost(upgrade: Upgrade, level: number): number | null {
...
export function getUpgradeFlavor(
  upgrade: Upgrade | null | undefined,
  level: number,
