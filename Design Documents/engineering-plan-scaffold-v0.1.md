...
game/
  ...
  src/
    ...
    scene/                            ── R3F / WebGL2 ──
      CosmicCanvas.tsx                the single <Canvas>; logarithmicDepthBuffer; Bloom; camera rig
...
1. **Lift the renderer config into `CosmicCanvas.tsx`.** The spike has a `<Canvas>` per scene (`MainScene`, `T1Scene`, `T2Scene`, `T5Scene`) each redefining camera/dpr/gl. Consolidate to one `CosmicCanvas` that takes a `tier` prop, mounts the tier's scene from `scene/tiers/registry.ts`, and owns the shared rig (Bloom, background layers, camera). **Add `gl={{ logarithmicDepthBuffer: true }}`** (Decision 8) — the spike does not set it; extreme-scale depth precision wants it before T5+ scenes with huge distance ranges land.
...
4. **`logarithmicDepthBuffer` + Bloom interaction (Decision 8).** The spike does NOT set `logarithmicDepthBuffer` and uses a hand-rolled `UnrealBloomPass` via `useFrame` priority (the `@react-three/postprocessing` + R3F 9 incompatibility, per NOTES.md). Enabling log-depth can interact with custom postprocessing passes (depth precision in the composer). *Mitigation:* enable log-depth in G4 and re-verify bloom on the absorbed scene before declaring G4 done; the NOTES.md `rm -rf node_modules/.vite` optimize-deps gotcha also applies when the postprocessing imports move.
