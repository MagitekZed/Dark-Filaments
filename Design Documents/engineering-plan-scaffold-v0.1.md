...
- **Content authoring — flavor text.** Clinical upgrade-card one-liners (~50–90 lines), first-purchase narrator lines beyond what the spike mocked, tier-up lines T4–T11. Attach: `data.ts` desc fields (narrator register) + `game/src/ui/clinicalDescriptions.ts` (clinical register) + `NarratorSurface` content. *(T1–T3 first-purchase + tier-up lines exist in the spike's `T2NarratorSurface` and can ride along where already authored.)* **Update (2026-06-02): the T1–T3 clinical card one-liners are now DONE** — 25 lines written + wired UI-side at `game/src/ui/clinicalDescriptions.ts`, locked in voice-samples.md §Clinical register. T4+ clinical lines remain TO-WRITE; tier-up lines T4–T11 still pending.
- **T5–T11 numerical calibration.** The engine ports with the current data.js values (T1–T4 locked, T5+ stale-pending-retune). v0.1 plays the T1–T2 arc; higher tiers mount scenes but are not numerically tuned. Attach: the existing sim-tuner workflow against the `Prototype/` tool, then port locked numbers into `game/src/engine/data.ts` (and re-run parity). *(Update 2026-06-02: T3 Dwarf Spheroidal is the first authored scene attached after the absorbed spike scenes — engine T3 was already parity-locked, so that was render/visual work only; it made the `useNamedOneShots`/`useStackableDensity` hooks real. T4 scene + T5+ numerical retunes remain pending.)*
