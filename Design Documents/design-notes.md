...
**Version 0.13 · Last updated 2026-05-21**

## Changelog

- **0.14 (current)** — **2026-05-25 menu/start music audio note (creative-director audio consultation).** §12 Tone & Atmosphere gained a new "Menu / start music" subsection directly beneath the Audio chain block: the title/menu track is the warm head of the slow-crossfading ambient chain — the CMB hum at its warmest and most present, before the descent; the title is the ceremonial first-entry threshold for a new universe (returning players land straight in the game, per the 2026-05-22 return-straight-in decision); instrumental space-ambient drone, no beat/melody/resolution, loopable under the ~2-min crossfade; must hold warmth + patient awe only, never telegraph cooling/sadness/resolution/peak-swell. Includes a "Generation prompt (Suno)" block (working title *The Oldest Light* + alternates + the exact style prompt, so the artifact is reproducible) and a forward-pointer proposing the recurring CMB-hum sonic motif as a deliberate authoring constraint across the Suno chain (to scope with engineering-director when the audio chain is built). Noted lightly as the seed for a future consolidated "Audio" section. No prose or slate changes elsewhere.
- **0.13** — **2026-05-21 React 19 lock (rules-guardian-vetted tech-stack revision; same-day sibling to the WebGL2 renderer lock).** §13 Tech Stack UI line changed from "React 18 + TypeScript" to "React 19 + TypeScript". React 19 matches the proven scene experiment (`experiments/galaxy-spike/` running React 19 + React Three Fiber 9 + drei 10 + three 0.184, hardware-verified on a Samsung S22 Ultra) and supersedes the original React-18 pin; React Three Fiber 9 targets React 19. rules-guardian confirmed no load-bearing rule depends on React 18 — renderer/version-only revision, no design intent changed. (Captured scaffold gotcha unaffected: `@react-three/postprocessing` 3.0.4 is incompatible with R3F 9 + React 19; the spike ships a hand-rolled bloom workaround instead — see engineering-plan-scaffold-v0.1.md.)
- **0.12** — **2026-05-21 renderer lock to WebGL2 (rules-guardian-vetted tech-stack revision).** §13 Tech Stack renderer line changed from "Three.js with WebGPU/TSL" to "Three.js (WebGL2) via React Three Fiber" with the conditional framing (WebGPU/TSL reserved for graphical needs that prove unbuildable in WebGL2; no dual-renderer or fallback path maintained). §16 Risks: the "WebGPU adoption / test the WebGL fallback path" item rewritten as resolved — single-renderer decision makes the fallback-test risk moot; WebGL2's ~97-98% mobile reach removes the adoption risk. "Three.js's logarithmic depth buffer" language in §13 kept verbatim (WebGL2-native feature). No design intent changed; renderer-API-only revision.
- **0.11** — **2026-05-14 UX workstream closing pass.** §18 5th idea-capture entry added: "Dim points + Fork C as one design idea" — the Causal Connections mystery number and the Inventory artifact reframed as one composition presented in two parts, separated by the endgame fade; canonical presentation surface is visual-design.md §9 hybrid composition. §16.5 cosmological time-arc subsection gained a forward pointer to visual-design.md §9: the time-arc gains a presentation surface in the Inventory's curated spiral layout (T1 near screen center, T11 at periphery), encoded spatially (radial position by tier) and temporally (brightness-as-recency across the deep field of dim points). No other prose or slate changes.
...
*(Creative-director audio consultation, 2026-05-25. Seed for a future consolidated "Audio" section if/when audio gets a focused workshop.)*

**Menu / start music.** The title/menu track is the warm head of the slow-crossfading ambient chain (§12) — the cosmic microwave background hum at its warmest and most present, before the descent. It sits at the beginning end of the emotional curve: full, rich, and beautiful, with the descent only latent. Because returning players now land straight in the game, the title screen is the *ceremonial first-entry threshold* for a new universe — a held breath before a weeks-long slow burn, with the live warm golden solar system rendered behind it. The track is instrumental space-ambient drone, not a structured song: a single sustained, breathing low hum with slow sub-bass swells, distant glassy bells and bowed pads overhead, immense cathedral reverb, no beat, no melody, no resolution — one evolving, loopable texture authored to fade seamlessly under the ~2-minute Web Audio crossfade. The CMB hum is the chain's sonic signature: the same low tone the menu carries at full warmth is the tone the rest of the game spends the entire arc cooling — swelling to awe at the peak (T6 Local Group), thinning and chilling through Act 2, and dying back into silence at the endgame ("the first light is dying back into the dark"). The menu must hold warmth and patient awe only; it must not telegraph cooling, sadness, resolution, or the swell reserved for the peak — the descent is felt later, not previewed at the door.

#### Generation prompt (Suno)

Working title: *The Oldest Light* (alternates: *Thirteen Billion Years*, *The Ancient Warmth*). "Surface of Last Scattering" was considered and reserved for a later peak/endgame cue — the word *Last* telegraphs ending at the threshold, where the menu wants unspoiled warmth.

Suno style prompt (instrumental; exclude drums/percussion/beat/vocals/melody): "Instrumental space ambient / drone — the cosmic microwave background as a single held, warm, thirteen-billion-year hum. A deep sustained sine-and-cello drone foundation, just barely breathing, with slow sub-bass swells that rise and recede over a minute or more. Sparse, distant glassy bell tones and bowed-string pads drift far overhead, never quite forming a melody. Immense weightless cathedral reverb; everything sounds vast and unhurried, as if heard from inside something enormous. No percussion, no beat, no rhythm, no chord progression, no resolution — one slowly evolving texture that only ever deepens. The mood is patient cosmic awe, vast and beautiful, with the faintest undertow of impermanence held far beneath the warmth, never surfacing. Analog warmth, subtle tape hiss, gentle low-frequency air. Seamless, loopable, no fade-in or fade-out — it should feel like a sound that was already playing before it began."

**Forward-pointer — recurring-hum sonic motif (proposed).** The single low CMB-hum tone the menu carries at full warmth should be the same tone the whole game cools across the arc (menu full warmth → peak max amplitude → Act 2 thinned/chilled → endgame silence), which means the hum should be a deliberate authoring constraint across the Suno chain. To be scoped with engineering-director when the audio chain is built.
...
- **Three.js (WebGL2) via React Three Fiber** for the 3D cosmic scene (WebGPU/TSL reserved for specific graphical needs that prove unbuildable in WebGL2; no dual-renderer or fallback path maintained in the meantime)
...
- **Renderer scope (resolved 2026-05-21).** The renderer is locked to WebGL2 (standard Three.js via React Three Fiber); WebGPU/TSL is reserved only for specific graphical needs that prove unbuildable in WebGL2. No dual-renderer or fallback path is maintained — the prior "test the WebGL fallback path before committing to WebGPU-only effects" risk is moot under the single-renderer decision. WebGL2's ~97-98% mobile reach removes the adoption risk that motivated this item.
