...
### Tier 3 — Dwarf Spheroidal *(implemented v0.1 scene — first cut, not a final aesthetic lock)*
**Scene:** A diffuse, gas-poor swarm of old, metal-poor Population II stars — warm reds and ambers, no nebula glow (an ancient population), low surface brightness. The cloud is **pressure-supported, not rotating**: it does not spin like a disc or galaxy, it is *held*, breathing almost imperceptibly. At this scale the structure IS the player (the T3 "embedding" tier-up line — *the stars we can count are the small part; the rest holds*). The dark-matter well sits at the origin; the visible swarm is the small luminous part of a much larger inferred mass.

**Tap reactivity:** A tap pulls matter toward the cloud's gravitational center — the dark-matter well at the origin — via the pull-particle feedback. The visible response is gravitational, not a brightening of the dark mass itself.

**SD-2 in this tier (load-bearing decision):** the visible cloud density is driven by the **luminous old population (Population II) only.** Buying **Subhalo** — the dark-matter stackable — never adds visible stars; the dominant Subhalo investment surfaces only as a strengthening **lensing signature** (a faint, non-luminous Einstein-ring distortion). This makes the tier thesis literal: the stars you can count stay the small part, and the rest is inferred, not rendered.

**Per-upgrade visuals (all real cosmology):**
- **RR Lyrae** — pulsating standard-candle variables embedded in the cloud.
- **Velocity Dispersion** — isotropic stellar-motion vectors (the dark-matter smoking gun), drawn as kinematic tracers.
- **Subhalo** — a faint, non-luminous Einstein-ring lensing signature (SD-2 inferred-not-rendered; deliberately subtle — it is the hidden channel).
- **Orphan Stream** and **Sagittarius Stream** — cold tidal stellar streams as long thin arcs; Sagittarius is the grand wrapping one (the T3→T4 gate one-shot, foreshadowing the arm).
- **Sculptor Dwarf** and **Draco Dwarf** — companion dwarfs; Draco is dark-dominated, rendered as a sparse star scatter with its own faint lensing halo ("the dark matter has a name").

This tier is the first authored scene in the `game/` scaffold after the absorbed spike scenes (committed 2026-06-02; engine T3 was already parity-locked, so this is render/visual work only). It generalizes the T1/T2 pattern: the two scene content hooks — `useNamedOneShots` (named one-shot upgrades appear as scene structures on purchase; the CD-7 mechanism made generic via a name→component registry) and `useStackableDensity` (stackables drive aggregate scene density) — are now REAL here, having been STUBs through G4-G5.

**Status (honest):** accepted as a strong **first cut** ("it looks good") and the work moved on; this is **not** a final aesthetic-direction lock for the tier (that remains out of v0.1 scope). The **Subhalo lensing ring is deliberately subtle** (it is the hidden channel) and its legibility-vs-subliminal balance remains an open call. The first patient-universe-return tier; the scene rewards returning calmly to find the structure quietly there.
