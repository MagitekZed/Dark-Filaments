...
import { PLAYER_PLANETS, FIELD_STARS } from './T2Scene'
...
// ─── Stable references ────────────────────────────────────────────────
//
// Module-level constants — every prop passed below that is an array or
// object literal MUST be a stable reference, or its consumer's useMemo
// deps will invalidate on every 60fps re-render and rebuild geometry
// per frame. This is the single biggest performance trap in this file.
...
// Bucket the 12 field stars into close/mid/far batches by camera-space
// distance at the back-half framing (camera ~[0, 8, 52]). Each batch
// mounts at its own threshold and scale-fades into the scene. Indices
// match FIELD_STARS order in T2Scene.tsx.
...
interface T1ToT2TransitionProps {
  onComplete: () => void
}

export function T1ToT2Transition({ onComplete }: T1ToT2TransitionProps) {
  // Hard timeout outside the Canvas so it fires even if useFrame stalls.
  ...
  return (
    <Canvas
      camera={{ position: [...T1_CAMERA.position], fov: T1_CAMERA.fov }}
      dpr={[1, 2]}
      gl={{ antialias: true }}
    >
      <color attach="background" args={['#000']} />
      <SceneContent fireComplete={fireComplete} />
    </Canvas>
  )
}

// ─── Inner driver — lives inside the Canvas ────────────────────────────

function SceneContent({ fireComplete }: { fireComplete: () => void }) {
  const { camera, clock } = useThree()
  const [progress, setProgress] = useState(0)
  const startTimeRef = useRef<number | null>(null)
  const completedLocalRef = useRef(false)

  // Refs for direct-mutation tweens (avoid React re-render for transforms)
  ...
  // Split planets once (stable references)
  ...
  useFrame(() => {
    ...
    // 2. Camera FOV — only after the back-half window opens
    ...
    // 3. Sun group — position + scale (radius via group scale)
    ...
    // 5. MiniPlanets at player-Sun — scale-in (when mounted)
    ...
    // 7. Close T1 nebula — live material opacity (no re-render)
    ...
    // Trigger one re-render per frame for state-driven props (opacity,
    // density, bloom). Children with stable useMemo deps don't rebuild.
    setProgress(p)
  ...
  // ─── Derived prop values (state-driven; cheap re-render) ─────────────
  ...
  // Prominence + coronal-loop particle sizes tween down with the Sun.
  // The Sun group's scale shrinks geometry (arc heights, jitter offsets)
  // but pointsMaterial.size is in world units — sizeAttenuation handles
  // distance falloff but NOT the local geometry shrink. Without tweening
  // here, particles look proportionally fat at the T2-scale Sun.
  // Targets match T2Scene.tsx's scaled values (0.042 * 0.55 = 0.0231,
  // 0.020 * 0.55 = 0.011).
  const sunShrinkT = cubicInOut(
    subWindow(progress, WINDOWS.sunMove[0], WINDOWS.sunMove[1]),
  )
  ...
  return (
    <>
      ...
      {/* Far nebula — both scenes share this region; persistent. */}
      <Nebula
      ...
      {/* Close T1 nebula — fades out via materialRef opacity */}
      <Nebula
      ...
      <group ref={sunGroupRef}>
        ...
        {/* Coronal loops — particleSize tweens 0.020 → 0.011 */}
        <PlasmaArcs
          starRadius={T1_SUN_RADIUS}
          arcs={18}
          particlesPerArc={280}
        ...
        {/* MiniPlanets — mount late, scale-in inside the Sun group so
            they ride along through the Sun's position move. */}
        {progress > WINDOWS.miniPlanetsMountAt && (
          <group ref={miniPlanetsGroupRef} scale={0}>
            {PLAYER_PLANETS.map((pp, i) => (
              <MiniPlanet key={i} {...pp} />
            ))}
          </group>
        )}
      ...
      {/* T1 planets — split into two groups so the inner rockies fade
          faster than the gas giants (the gas giants are more visually
          load-bearing through the early pull). */}
      ...
      {/* Inner asteroid belt (Mars-Jupiter gap) */}
      <AsteroidBelt
        innerRadius={8.0}
        outerRadius={9.1}
        count={1000}
        thickness={0.18}
        midOrbitPeriod={160}
        opacity={asteroidBeltOpacity}
      />

      {/* Kuiper belt — icy palette, persists slightly longer than the inner belt */}
      <AsteroidBelt
        innerRadius={14.5}
        outerRadius={16.8}
        count={1500}
      ...
      {/* Zodiacal light — fades fastest (camera passes through it) */}
      <ZodiacalLight
        innerRadius={1.5}
        outerRadius={9.5}
        count={1500}
        thickness={0.10}
        opacity={zodiacalOpacity}
      />

      {/* Comet — unmount-after-threshold. Its tail is world-space, so a
          scale-wrapper would not shrink it. */}
      ...
      {/* Field stars — staggered batches, each scale-fades in */}
